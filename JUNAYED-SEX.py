...
WHATSAPP = '01608107921'
faceb0ok = 'Junayed Hasnat Mridul'
notice = 'Working Both Data + WiFi '
...
def cek_akun():
	...
	except:sys.exit(f" [{M}>{P}] tidak ada JUNAYEDðŸ’” ok")
	for x in ok:
		nom.append(x)
		no+=1
...
def cek_HASAN():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] cek JUNAYEDðŸ’š akun ok\n [{hh}2{P}] cek HASANðŸ’” akun cp\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		except:sys.exit(f" [{M}>{P}] tidak JUNAYEDðŸ’š ok")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('OK/'+x,'r').readlines()
...
def cari_nama(link):
	...
	try:
		link = r.find('a',string='Lihat JUNAYEDðŸ’” Selanjutnya').get('href')
		if(link):
			print('\r sedang dump %s id'%(len(dump)),end=" ")
			sys.stdout.flush()
			cari_nama(link)
